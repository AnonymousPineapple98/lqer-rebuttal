...
from quanto.tensor import PackedTensor
...
class _FromFloat4Func(torch.autograd.Function):
    """Cast from FP4 to other dtype"""
    ...
    def forward(
        ...
        if dtype is None:
            dtype = tensor.dtype
        data = tensor._data.unpack().contiguous().view(1, -1).detach()
...
class _ToFloat4Func(torch.autograd.Function):
    """Cast to FP4 from other dtype"""
    ...
    def forward(
        ...
        for key, val in default_kwargs.items():
            if key not in init_kwargs:
                init_kwargs[key] = val
        return Float4Tensor(**init_kwargs)
...
class Float4Tensor(torch.Tensor):
    ...
    def __new__(
        ...
        *,
        ...
        self._data = PackedTensor.pack(data)
        ...
        **kwargs,
        ...
        for key, val in default_kwargs.items():
            if key not in kwargs:
                kwargs[key] = val
        return Float4Tensor(data=data, fp8_attrs=fp8_attrs, **kwargs)
    ...
    def __repr__(self):
        return (
            ...
            f"data={self.from_float4(dtype=self.dtype)}"
            ")"
        )

    def from_float4(self, dtype: Optional[torch.dtype] = None) -> torch.Tensor:
        """
        ...
        """
        return _FromFloat4Func.apply(self, dtype)
    ...
    def float(self) -> torch.Tensor:
        return self.from_float4(dtype=torch.float32)

    def bfloat16(self) -> torch.Tensor:
        return self.from_float4(dtype=torch.bfloat16)

    def half(self) -> torch.Tensor:
        return self.from_float4(dtype=torch.float16)

    def cpu(self) -> torch.Tensor:
        return self.from_float4().cpu()

    def clone(self) -> Float4Tensor:
    ...
    def transpose(
        ...
        *,
        ...
        out = self._transpose
        if out is None:
            out = Float4Tensor.make_like(
                self,
                data=tex.fp8_transpose(
                    self._data.unpack().contiguous(),
                    self._fp8_dtype,
                ),
            )
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs=None):
        ...
        if func == aten.copy_.default:
            ...
            dst_is_fp8 = isinstance(dst, Float4Tensor)
            src_is_fp8 = isinstance(src, Float4Tensor)
            if dst_is_fp8 and src_is_fp8:
                ...
                if dst._fp8_dtype == src._fp8_dtype:
                    ...
                    if dst._fp8_meta is not None:
                        if src._fp8_meta is None:
                            src_min, src_max = src.from_float4().aminmax()
                            ...
                            src_amax = src._fp8_meta[fp8_meta_key].amax_history[0][
                                fp8_meta_index
                            ]
                        fp8_meta_key = FP8GlobalStateManager.get_meta_tensor_key(
                        ...
                        dst_amax = dst._fp8_meta[fp8_meta_key].amax_history[0][
                            fp8_meta_index
                        ]
                        torch.maximum(src_amax, dst_amax, out=dst_amax)
                else:
                    dst.copy_(src.from_float4())
                ...
                # Invalid case
                raise RuntimeError(
                    "Using Float8Tensor copy logic, but no Float8Tensor found"
                )
        ...
        if func == aten.slice.Tensor:
            ...
            data_slice = PackedTensor.pack(data_slice)
            return Float4Tensor.make_like(tensor, data=data_slice)
        ...
        def maybe_unwrap(t):
            if isinstance(t, Float4Tensor):
                return t.from_float4()
            return t
    ...
    def __reduce_ex__(self, protocol: int) -> tuple:
        ...
        return (
            Float4Tensor._make_in_reduce_ex,
            (self._data, self._fp8_dtype, self._scale_inv, self.dtype),
        )

    def _get_data(self) -> Float4Tensor:
